...
\textbf{(a)} Theorem: ``Every natural number is tall or wide, but not both.'' (Proof method: proof by cases.)
...
\bigskip
\textbf{(b)} Theorem: ``There exists a happy natural number between $10$ and $1000$ that has no happy proper divisors.'' (Proof method: choose $25$ as the example.)
...
\bigskip
\textbf{(c)} Theorem: ``There are no devious natural numbers.'' (Proof method: regular induction, with base cases $0$ and $1$.)
...
\medskip
\textbf{(a) Befuddled Brynmor's mistake.}
...
\medskip
\textbf{(b) Erroneous Erik's mistake.}
...
\medskip
\textbf{(c) Overzealous Zach's mistake.}
...
\medskip
\textbf{(a)} Tracing from $(5,22,0)$ (target: $5\cdot 22 = 110$):
\[
...
\medskip
\textbf{(b)} \textbf{Invariant.} Let $P(r,s,a) := $ ``$rs + a = xy$.'' We show $P$ is an invariant of the state machine: true at the start state, and preserved by every transition.
...
\medskip
\textbf{(c)} \textbf{Final states.} Transitions are defined exactly when $s > 0$, so a state has no outgoing transition (is final) precisely when $s = 0$. Thus the final states are exactly the triples of the form $(r, 0, a)$ for $r, a \in \N$.

\medskip
\textbf{(d)} \textbf{Partial correctness.} Let $(r_f, s_f, a_f)$ be any reachable final state. By part (c), $s_f = 0$. Since $P$ is an invariant (part (b)) and holds at the start state, it holds at every reachable state, including this one:
\[
...
\medskip
\textbf{(e)} \textbf{Termination.} Let $\mathrm{bitcount}(n) := \lceil \log_2(n+1) \rceil$ for $n \in \N$. We show $\mathrm{bitcount}(s)$ strictly decreases — in fact, decreases by exactly $1$ — with every transition.
...
\medskip
\textbf{(f)} \textbf{Time bound.} By part (e), $\mathrm{bitcount}(s)$ decreases by exactly $1$ at every step, starting from $\mathrm{bitcount}(y)$ at the initial state $(x,y,0)$ and ending at $\mathrm{bitcount}(0) = 0$ at the final state. Hence, the number of transitions taken is exactly
\[
